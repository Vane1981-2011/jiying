...
/**
 * 检查结果
 * @typedef {{ severity: string, category: string, issue: string, evidence: string, suggestedFix: string }} CheckResult
 */
 ...
 * @param {object} options.plan - Planner 输出
 * @param {Array} options.creatorResults - Creator 各子任务结果
 * @param {object|null} options.review - Reviewer 输出
 * @returns {{ passed: boolean, results: CheckResult[], criticalCount: number, warningCount: number }}
 */
export function runSelfReview({ intent, plan, creatorResults, review }) {
  const results = [];
  const checks = [
 ...
 * 🟡 Warning: 有 warn 状态
 */
function checkConstitutionStatus(creatorResults) {
 ...
 * 🟡 Warning: 有子任务被跳过或无内容
 */
function checkAgentAllParticipated(creatorResults) {
 ...
 * 🟡 Warning: 覆盖率 < 80%
 */
function checkContentCoverage(creatorResults) {
  if (!creatorResults?.length) return null;
...
function checkPlaceholderStrings(creatorResults) {
  if (!creatorResults?.length) return null;
 ...
 * 🟢 Info: 评分 >= 4/5
 */
function checkReviewerScoring(review) {
  if (!review) return {
    severity: SEVERITY.WARNING,
    category: '审查评分',
 ...
 * 🟡 Warning: 部分子任务缺少假设
 */
function checkAssumptionsDeclared(creatorResults) {
  if (!creatorResults?.length) return null;
 ...
 * 🟡 Warning: 检测到内容重复
 */
function checkContentDeduplication(creatorResults) {
