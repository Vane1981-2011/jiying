 ...
 * - Evolve: 将改进沉淀为知识资产
 *
 * 存储：优先使用 IndexedDB（上限 50000 条），不可用时回退到 localStorage（上限 500 条）
 */

import { openDB, put, get, clearStore } from '../utils/idb-storage.js';

const DB_NAME = 'jiying-evolution-db';
const DB_VERSION = 1;
const STORE_NAME = 'evolution';
const EVOLUTION_KEY = 'jiying-evolution-metrics';

// 之前 localStorage 上限为 500 条，IndexedDB 大幅提升至 50000 条
const MAX_ITEMS_IDB = 50000;
const MAX_ITEMS_LOCALSTORAGE = 500;
...
async function saveHistory(history: Snapshot[]): Promise<void> {
  try {
    const db = await getDb();
    if (db) {
      await put(db, STORE_NAME, EVOLUTION_KEY, history.slice(-MAX_ITEMS_IDB));
      return;
    }
  } catch {
    // IndexedDB 写入出错，降级
  ...
  // --- localStorage 回退 ---
  try {
    localStorage.setItem(EVOLUTION_KEY, JSON.stringify(history.slice(-MAX_ITEMS_LOCALSTORAGE)));
  } catch {
    // 静默失败（localStorage 满等情况）
  }
}
