...
/** 放大风险级别 */
export type AmplificationRiskLevel = 'critical' | 'warning' | 'info';

/** 放大风险条目 */
export interface AmplificationRisk {
  level: AmplificationRiskLevel;
  issue: string;
  amplification1000x: string;
}

/** 利害关系人 */
export interface Stakeholder {
  who: string;
  benefitOrHarm: 'benefit' | 'harm';
  severity: 'high' | 'medium' | 'low';
}

/** 不可逆性级别 */
export type IrreversibilityLevel = 'reversible' | 'hard_to_reverse' | 'irreversible';

/** 最终裁决 */
export type Verdict = 'approve' | 'caution' | 'block';

/** 伦理检查输入 */
export interface EthicsCheckInput {
  content: string;
  goal: string;
  type?: 'text' | 'decision' | 'analysis';
}
 ...
 * 执行伦理放大效应检查
 *
 * @param input.content - Agent 输出内容
 * @param input.goal - 原始意图
 * @param input.type - 内容类型
 */
export function ethicsAmplificationCheck(input: EthicsCheckInput): EthicsCheckOutput {
  const { content, goal, type = 'analysis' } = input;
  const results: EthicsCheckOutput = {
    passed: true,
    amplificationRisks: [],
    stakeholders: [],
  ...
  const irreversiblePatterns: Array<{ pattern: RegExp; level: IrreversibilityLevel }> = [
