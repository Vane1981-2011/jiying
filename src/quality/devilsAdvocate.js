 ...
 *
 * @param {object} input
 * @param {string} input.content - 要批判的内容
 * @param {string} [input.context] - 上下文信息
 * @param {object} [input.assumptions] - 已知的假设列表
 * @param {'quick'|'deep'} [input.mode='quick'] - 检查深度
 * @returns {{
 *   challenged: boolean,
 *   hiddenAssumptions: Array<{assumption, challenge, severity}>,
 *   biasesDetected: Array<{bias, evidence, severity}>,
 *   counterEvidence: Array<{claim, counter, strength}>,
 *   reverseArgument: string,
 *   residualRisk: Array<{risk, probability, impact}>,
 ...
 */
function mineAssumptions(content, context, isDeep) {
  const assumptions = [];
 ...
 * 偏见检测 — 对照认知偏见清单检查
 */
function detectBiases(content, context, isDeep) {
  const detected = [];
  const text = content.toLowerCase();
  const ctxText = (context + ' ' + content).toLowerCase();
 ...
 */
function findCounterEvidence(content, knownAssumptions, isDeep) {
  const evidence = [];
  const text = content.toLowerCase();
 ...
 * 构建反向论证 — 为相反的立场构建最强论证
 */
function buildReverseArgument(content, assumptions, context) {
 ...
 * 残余风险评估
 */
function assessResidualRisk(assumptions, biases, counterEvidence) {
  const risks = [];
 ...
 * 计算综合置信度
 */
function calculateConfidence(assumptions, biases, counterEvidence, isDeep) {
  let score = 5;
...
function highSeverityCount(items) {
  return items.filter((i) => i.severity === 'high').length;
}

export { COGNITIVE_BIASES };
